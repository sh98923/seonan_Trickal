...
public class CharacterGraphics : MonoBehaviour
{
    ...
    private float _statusEndTime = 0.0f; // 상태 이상 끝나는 절대 시간
    ...
    private IEnumerator StatusColor()
    {
        ...
        {
            case HitType.Cold:
                statusColor = _coldHitColor;
                break;
            case HitType.Stone:
                statusColor = _stoneHitColor;
                break;
        }

        _currentColor = statusColor;

        SetSpriteColor(statusColor);

        // 느려짐 또는 굳음 (애니메이션 설정)
        _character.SetAnimatorSpeed(animSpeed);

        //Time.time을 쓰면 슬로우나 석화가 연장이 가능함
        while (Time.time < _statusEndTime)
        {
            // 캐릭터가 죽으면 즉시 색 복원 후 종료
            if (_character.CurState == CharacterState.Dead)
            {
                _isStatusActive = false;
                _character.SetAnimatorSpeed(1.0f);
